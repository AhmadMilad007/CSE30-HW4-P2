...
extern unsigned int pokeFP2float(unsigned int);
 ...
 * encoded in the Pokedex-FP format, convert them into 32-bit
 * floats, and print out the resutls to stdout. It also checks
 * that registers are left unmodified by the pokeFP2float() call.
 */
int main(int argc, char *argv[]) {
	...
	while(fread(&pokeFP, 1, 1, fp)) {
		...
		cvt.i = pokeFP2float((uint32_t)pokeFP);
		assert(l0 == 0xffffff0);
		assert(l1 == 0xffffff1);
		assert(l2 == 0xffffff2);
		assert(l3 == 0xffffff3);
